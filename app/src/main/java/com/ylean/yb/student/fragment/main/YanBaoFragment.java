 ...
 * 燕宝人
 */
public class YanBaoFragment extends BaseFragment implements CultrueP.Face {
    ...
    public void getNewsTitle(final List<NewsTitleBean.ListBean> list) {
        ...
        for (int i=0;i<listAll.size();i++){
            ...
            fragments[i]=new YanBaoChildFragment();
        }

        //给第一个fragment设置id
        ((YanBaoChildFragment)fragments[0]).setId(listAll.get(0).getId());
        ...
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ((YanBaoChildFragment)fragments[position]).setId(listAll.get(position).getId());
