 ...
 * 机构文化
 */
public class CultrueFragment extends BaseFragment implements CultrueP.Face {
    ...
    public void getNewsTitle(final List<NewsTitleBean.ListBean> list) {
        ...
        for (int i=0;i<listAll.size();i++){
             ...
             fragments[i]=new CultureChildFragment();
        }

        //给第一个fragment设置id
        ((CultureChildFragment)fragments[0]).setId(listAll.get(0).getId());
        ...
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ((CultureChildFragment)fragments[position]).setId(listAll.get(position).getId());
